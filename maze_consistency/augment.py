...
For a sampled training rollout, pick a switch time tau, keep its prefix h_tau, and let the model continue,
asking for a higher outcome than the original achieved. The continuation is IMAGINED: actions come from the
R-conditioned policy, next cells from the model's own dynamics head read in NOR mode (the unconditioned world
model -- conditioning the dynamics on a high requested R would favour lucky transitions). No environment
interaction produces the data; the setting stays offline.

Generation uses recorded prefixes, reward-bin definitions and learned dynamics only. Exact maze distances
and transitions are used by summarize() AFTER generation for evaluation, never for selection or labels.
Environment-stepped training buffers are refused.
...
100% of the random-walk ceiling is possible. RolloutBuffer.history compares imagined outcomes against the
random walk and reports hallucinations; real-maze policy evaluation is needed to establish improvement.
...
from .evaluate import make_action_logits, make_cell_logits, continue_rollout, rollout_diagnostics

KEYS = ("positions", "actions", "length", "reached")


def splice(params, action_logits, tok, maze, d, pool, n, rng, tau_max=None, request="above",
           max_tries=50, cell_logits=None):
    ...
    request="above": MODE uniform over all bins above the recorded outcome. request="highest": always K-1
    ("best" is a legacy alias for "highest", NOT a feasibility oracle). Only finished prefixes and rows
    already labelled K-1 are resampled. Requests may be infeasible; all generated outcomes are retained.

    Returns the dataset layout (positions, actions, length, reached -- so the outcome bin is the one ACHIEVED)
    plus tau, src, requested, orig_bin, orig_reached, achieved. Exact-maze diagnostics are computed separately
    after generation and cannot affect sampling or the training rows.

    cell_logits is required: training continuations must use a learned world model."""
    if cell_logits is None:
        raise ValueError("splice requires learned cell_logits; real-maze dynamics are evaluation-only")
    if request not in ("above", "highest", "best"):
        raise ValueError(f"request={request!r} not in above|highest (best is a legacy alias)")
    T, K = maze.T, maze.K
    ...
    for _ in range(max_tries):
        ...
        ok = (L > tau) & (orig < K - 1)
        if ok.any():
            picks.append((idx[ok], tau[ok], orig[ok]))
            have += int(ok.sum())
        if have >= n:
            break
    if have < n:
        raise RuntimeError(f"found only {have} of {n} unfinished prefixes with recorded outcomes below bin {K - 1}")
    idx, tau, orig = (np.concatenate(z)[:n] for z in zip(*picks))
    req = rng.integers(orig + 1, K) if request == "above" else np.full(n, K - 1)
...
def summarize(maze, b, gt=None) -> dict:
    ...
    diag = rollout_diagnostics(maze, b, gt=gt)
    out = dict(n=int(len(b["length"])), mean_tau=float(b["tau"].mean()),
               ...
               far_own_best=diag["far_own_best"])
    bad, imp = diag["n_bad"] > 0, b["achieved"] > b["orig_bin"]
    out["halluc"] = float(bad.mean())                                    # rollouts with >= 1 impossible move
    out["halluc_improved"] = float(bad[imp].mean()) if imp.any() else 0.0
    out["improved_rw"] = float(diag["p_improve_rw"].mean())
    out["got_request_rw"] = float(diag["p_request_rw"].mean())
    return out
...
class RolloutBuffer:
    ...
    def __init__(self, model, tok, maze, d, pool, size=512, refresh_every=500, start_after=1000, tau_max=100,
                 request="above", dynamics="model", seed=0, log=print):
        if dynamics != "model":
            raise ValueError("training buffers require dynamics='model'; real-maze dynamics are evaluation-only")
        ...
        self.cell_logits = make_cell_logits(model, tok)
    ...
    def maybe_refresh(self, params, step) -> bool:
        ...
        self.data = splice(params, self.action_logits, self.tok, self.maze, self.d, self.pool, self.size,
                           self.rng, self.tau_max, self.request, cell_logits=self.cell_logits)
        self.last = step
        ...
        self.log(f"  rollouts@{step}: improved {s['improved']:.2f} (random walk {s['improved_rw']:.2f})  "
                 ...
                 f"halluc {s['halluc']:.2f} (improved {s['halluc_improved']:.2f})")
        return True

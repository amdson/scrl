...
The real maze is used for one thing: counting imagined transitions it would not allow (n_bad), as a
diagnostic. Asking for high reward selects for trajectories where the world model errs in a helpful
direction -- a hallucinated shortcut earns a high bin -- so summarize() reports the hallucination rate among
the rollouts that improved, not just overall. dynamics="env" steps the real maze instead (online
interaction, useful only as an upper bound).
...
from .evaluate import make_action_logits, make_cell_logits, continue_rollout
...
def splice(params, action_logits, tok, maze, d, pool, n, rng, tau_max=None, request="above", gt=None,
    ...
    cell_logits: imagine the next cells with the model's world model (see evaluate.continue_rollout); None steps
    the real maze. n_bad counts imagined transitions the maze would not allow."""
    ...
    out = dict(positions=ro["positions"].astype(np.int16), actions=ro["actions"].astype(np.int8),
               ...
               achieved=maze.outcome_bin(ro["length"], ro["reached"]).astype(np.int64), n_bad=ro["n_bad"])
    if gt is not None:
...
def summarize(maze, b) -> dict:
    ...
    bad, imp = b["n_bad"] > 0, b["achieved"] > b["orig_bin"]
...
class RolloutBuffer:
    ...
    def __init__(self, model, tok, maze, d, pool, size=512, refresh_every=500, start_after=1000, tau_max=100,
        ...
        if dynamics not in ("model", "env"):
            raise ValueError(f"dynamics={dynamics!r} not in model|env")
        ...
        self.cell_logits = make_cell_logits(model, tok) if dynamics == "model" else None
    ...
    def maybe_refresh(self, params, step) -> bool:
        ...
        self.data = splice(params, self.action_logits, self.tok, self.maze, self.d, self.pool, self.size,
                           self.rng, self.tau_max, self.request, gt=self.gt, cell_logits=self.cell_logits)
        self.last = step
